...
export default function LoginPage() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      ...
      login(account.user);
      router.replace(account.user.verified ? "/dashboard" : "/onboarding");
    }, 600);
  };
  ...
  return (
    <div className="w-full max-w-sm">
      ...
      <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-8">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <button
            type="submit"
            disabled={submitting}
            className="w-full bg-white text-zinc-950 py-2.5 rounded-lg font-bold text-sm hover:bg-zinc-200 transition-all active:scale-95 disabled:opacity-60 disabled:cursor-not-allowed mt-2"
          >
      ...
      <p className="text-center text-sm text-zinc-500 mt-6">
        No account?{" "}
        <Link href="/register" className="text-zinc-300 font-medium hover:text-white transition-colors">
          Create one
        </Link>
      </p>
    </div>
